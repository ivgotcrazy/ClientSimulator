...
{
    //Constructors
    SBlock(FS_UINT32 l = 0, FS_UINT32 r = 0) : m_uil(l), m_uir(r) {}
    //Copy Constructor
    SBlock(const SBlock& roBlock) : m_uil(roBlock.m_uil), m_uir(roBlock.m_uir) {}
    SBlock& operator^=(SBlock& b) { m_uil ^= b.m_uil; m_uir ^= b.m_uir; return *this; }
    FS_UINT32 m_uil, m_uir;
};
...
public:
    ...
    CBlowFish(FS_UINT8* ucKey, size_t n, const SBlock& roChain = SBlock(0UL, 0UL));

    //Resetting the chaining block
    void ResetChain() { m_oChain = m_oChain0; }

    // Encrypt/Decrypt Buffer in Place
    void Encrypt(FS_UINT8* buf, size_t n, FS_INT32 iMode = ECB);
    void Decrypt(FS_UINT8* buf, size_t n, FS_INT32 iMode = ECB);

    // Encrypt/Decrypt from Input Buffer to Output Buffer
    void Encrypt(const FS_UINT8* in, FS_UINT8* out, size_t n, FS_INT32 iMode = ECB);
    void Decrypt(const FS_UINT8* in, FS_UINT8* out, size_t n, FS_INT32 iMode = ECB);

    //Private Functions
private:
    FS_UINT32 F(FS_UINT32 ui);
    void Encrypt(SBlock&);
    void Decrypt(SBlock&);
...
private:
    ...
    SBlock m_oChain;
    FS_UINT32 m_auiP[18];
    FS_UINT32 m_auiS[4][256];
    static const FS_UINT32 scm_auiInitP[18];
    static const FS_UINT32 scm_auiInitS[4][256];
};

//Extract low order byte
inline FS_UINT8 Byte(FS_UINT32 ui)
{
    return (FS_UINT8)(ui & 0xff);
}

//Function F
inline FS_UINT32 CBlowFish::F(FS_UINT32 ui)
{
